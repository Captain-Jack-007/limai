...
### 10.4 docx 导出限制（Phase 7 首发版）

| 限制 | 说明 | 计划修复时间 |
|------|------|-------------|
| 引用编号非自动重排 | 用户在 Word 中删除某章后，后续 [N] 编号不会自动减一。Bookmark 跳转仍能工作，但显示的数字与实际位置不一致 | Phase 7 第二版引入 SEQ 字段 |
| 自动目录需手动刷新 | 首次打开 docx 后，需在 Word 中右键目录 → 更新域，才能显示章节列表 | 未来支持插入预生成的静态目录 |
| 可信度徽章在 WPS 中颜色可能略有差异 | docx 颜色定义为 hex，主流 Office 软件均支持，但旧版 WPS 可能渲染为黑色 | 不修复，需要用户更新 WPS |
| FootnoteReferenceRun 未启用 | 当前实现使用 InternalHyperlink 模拟引用，而非 Word 原生 footnote。原因：Phase 6 输出已是 [N] 数字角标，改为原生 footnote 需重新设计编号机制 | 视用户反馈决定 |
